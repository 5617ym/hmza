...
module.exports = async function (context, req) {
  ...
  try {
    ...
    const isImage = ct.startsWith("image/") || /\.(png|jpg|jpeg|webp|tiff|bmp)$/i.test(name);

    const isCsv = name.endsWith(".csv") || ct.includes("csv");
    const isXlsx = /\.(xlsx|xls)$/i.test(name) || ct.includes("spreadsheet") || ct.includes("excel");
    const isDocx = name.endsWith(".docx") || ct.includes("wordprocessingml") || ct.includes("msword");
    ...
    return send(400, { ok: false, error: "نوع ملف غير مدعوم حالياً", details: { fileName, contentType } });
  } catch (err) {
